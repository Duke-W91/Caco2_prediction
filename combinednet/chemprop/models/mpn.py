...
class MPNEncoder(nn.Module):
    ...
    def __init__(self, args: TrainArgs, atom_fdim: int, bond_fdim: int, return_weight: bool =False):
        """
        ...
        self.W_a = nn.Linear(self.hidden_size, 1, bias=self.bias)
    ...
    def forward(self,
        ...
        # Readout
        mol_vecs, atom_w_list = [], []
        for i, (a_start, a_size) in enumerate(a_scope):
            if a_size == 0:
                mol_vecs.append(self.cached_zero_vector)
            else:
                cur_hiddens = atom_hiddens.narrow(0, a_start, a_size)
                att_w = torch.sigmoid(self.W_a(cur_hiddens))
                att_hiddens = torch.mul(att_w, cur_hiddens)
                mol_vec = (cur_hiddens + att_hiddens)
                atom_w_list.append(att_w) # (num_atoms, hidden_size)
...
class MPN(nn.Module):
    ...
    def __init__(self,
        ...
        if args.mpn_shared:
            self.encoder = nn.ModuleList([MPNEncoder(args, self.atom_fdim, self.bond_fdim, self.return_weight)] * args.number_of_molecules)
        else:
            self.encoder = nn.ModuleList([MPNEncoder(args, self.atom_fdim, self.bond_fdim, self.return_weight)
    ...
    def forward(self,
        ...
        if self.atom_descriptors == 'descriptor':
            ...
            if self.return_weight:
                ...
                atm_w_list = [enc(ba, atom_descriptors_batch)[1] for enc, ba in zip(self.encoder, batch)]
            else:
            ...
            if self.return_weight:
                ...
                atm_w_list = [enc(ba)[1] for enc, ba in zip(self.encoder, batch)]
            else:
