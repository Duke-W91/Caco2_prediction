...
from chemprop.args import AttentionArgs, TrainArgs
...
@timeit()
def molecule_attention(args: AttentionArgs, smiles: List[List[str]] = None) -> List[List[Optional[float]]]:
    """
    ...
    args: Union[AttentionArgs, TrainArgs]
    ...
    if smiles is not None:
        ...
        full_data = get_data(path=args.test_path, smiles_columns=args.smiles_columns, target_columns=[], ignore_columns=[],
                             skip_invalid_smiles=False,args=args, store_row=True)
    ...
    # Load model
    print(f'Encoding smiles into an attention vector from a single model')
    if len(args.checkpoint_paths) != 1:
        raise ValueError("Attention generation only supports one model, cannot use an ensemble")

    model = load_checkpoint(args.checkpoint_paths[0], return_weight = args.return_weight, device=args.device)
...
#    assert len(test_data) == len(model_atts)
...
#        preds = model_atts[valid_index] if valid_index is not None else ['Invalid SMILES'] * total_hidden_size

#        fingerprint_columns=[f'att_{i}' for i in range(total_hidden_size)]
...
def model_attention(model: MoleculeModel,
...
#         mol_batch, features_batch, atom_descriptors_batch = batch.batch_graph(), batch.features(), batch.atom_descriptors()
        # Make predictions
        with torch.no_grad():
            batch_fp = model.featurize(mol_batch, features_batch, atom_descriptors_batch, atom_features_batch, bond_features_batch)
...
def chemprop_attention() -> None:
    ...
    """
    molecule_attention(args=AttentionArgs().parse_args())
