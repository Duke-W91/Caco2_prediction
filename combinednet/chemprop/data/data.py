...
class MoleculeDataset(Dataset):
    ...
    def rdkit_2d_normalized_features(self) -> List[np.ndarray]:
        """
        ...
        list_2dfeatvecs = []
        for d in self._data:
            smiles = Chem.MolToSmiles(d.mol[0], isomericSmiles=True) if type(d.mol[0]) != str else d.mol[0]
            generator = rdNormalizedDescriptors.RDKit2DNormalized()
            #generator = rdDescriptors.RDKit2D()
            features = generator.process(smiles)[1:]
            features = [0 if np.isnan(x) else float(x) for x in features]
            list_2dfeatvecs.append(features)

        return list_2dfeatvecs
